...
const UserDetailsModal: React.FC<UserDetailsModalProps> = ({ 
    user, allTeachers, onSave, onCancel, generateCode, isGeneratingCode,
    defaultPermissions, defaultCode, lockPermissions = false
}) => {
    const { t } = useLanguage();
    ...
    useEffect(() => {
        if(isNewUser) {
            ...
            if (defaultCode && !formData.code) {
                setFormData(prev => ({ ...prev, code: defaultCode }));
            } else if (!formData.code && !defaultCode && !isGeneratingCode) {
                // Only auto-generate if no default code provided
                handleGenerateCode();
            }
        }
    }, [isNewUser]);
    ...
    const handlePermissionChange = (permission: Permission, isChecked: boolean) => {
        if (lockPermissions) return; // Prevent changes if locked
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-60 flex justify-center items-center z-50 p-4">
            <div className="bg-white p-6 rounded-lg shadow-xl w-full max-w-3xl max-h-[90vh] overflow-y-auto space-y-4">
                ...
                {formData.permissions.includes('view_reports_for_specific_teachers') && !lockPermissions && (
                     <div className="border-t pt-4">
