...
export type Permission = 
  ...
  'create_self_evaluation'; // New permission
...
export interface SelfEvaluationReport extends BaseReport {
    ...
    questionsGlossaryPercentage: number;
    
    // Dynamic Text Fields
    programsAndSkills: string;
    strategiesExecuted: string;
    toolsUsed: string;
    sourcesUsed: string;
    tasksAccomplished: string; // التكاليف التي تم القيام بها
    testsDelivered: string;
    peerVisitsDone: string;
}

export type Report = GeneralEvaluationReport | ClassSessionEvaluationReport | SpecialReport | SelfEvaluationReport;

export type EvaluationType = 'general' | 'class_session' | 'special' | 'self_evaluation';
